...
Первый класс Tomato содержит 3 метода, в первом мы объявляем атрибуты (порядковый номер и базу стадий созревания), второй метод повышает стадию созревания у выбранного куста, а третий проверяет созрел ли куст.
...
Третий класс Gardener содержит 3 обычных метода и 1 статичный. Первый метод создаёт 2 атрибута (Имя садовника и растение), второй метод work - повышает степень созревания у всех растений на 1 (Ухаживает за томатами), третий метод harvest проверяет все ли томаты созрели, если все, то собироает их (очищает список кустов), если нет то выводит сообщение об этом, а статичный метод knowledge_base выводит справку.
